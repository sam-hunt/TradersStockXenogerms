...
│   ├── XenogermIdentity.cs             # Which preset a germ yields: comp, else gene-set inference (unit-tested)
...
**Preset identity resolution (`XenogermIdentity`):** the implant stamp and the ideology gate never read `CompXenotypeSource` directly — they ask `XenogermIdentity.Resolve`, which returns the comp's def when set and otherwise *infers* the preset whose gene list equals the germ's (`GenesMatch`, vanilla's `PawnIsCustomXenotype` rule: `passOnDirectly=false` genes ignored on both sides). Inference exists because comp state does not survive every path an item takes: ReSplice Core's xenogerm duplicator rebuilds its copy with `ThingMaker.MakeThing` and copies only `xenotypeName`/`iconDef`/genes, so a duplicated Hussar germ used to implant as a custom "hussar" (same genes, no ideology recognition, no germline retarget). Gene set first, name as tie-breaker only: two loaded presets sharing one gene list are split by `xenotypeName` against label/defName (case-insensitive), and an unbreakable tie resolves to null (vanilla behaviour). Name-first was rejected deliberately — a player who assembles Hussar's genes one genepack at a time ends with a germ that is a Hussar in every way vanilla can measure, whatever they typed last. **Inference only names what vanilla cannot:** before any preset is considered, a non-inheritable template in `Current.Game.customXenotypeDatabase` (the xenotype-editor templates seeded from the scenario's starting pawns and the chosen ideos' xenotype precepts) that matches the genes claims the germ and the resolver returns null — vanilla implants untouched and the pawn's `CustomXenotype` resolves to the player's template. A player who authored a preset clone and deliberately did not pick the preset has expressed a preference (whatever the reason — an interaction with another mod, say) and the mod does not overrule it; inheritable templates don't count because they match against endogenes, which a xenogene implant never writes. One asymmetry follows: a trader-sold germ stamps its def via the comp regardless, so in a game holding such a clone a bought germ and its comp-less duplicate diverge — the player's template winning, by design. Such a template can only be authored in the xenotype editor (`Dialog_CreateXenotype`, the on-disk files `Precept_Xenotype`'s picker offers), never at the gene assembler, which saves xenogerm *templates* (`CustomXenogermUtility`), a different file kind that never reaches an ideo or the database. Consequence by design: a hand-assembled gene-for-gene copy of a preset implants as that preset. Gene-less presets are never inferred (an empty germ must not become a Baseliner conversion by accident — that item is only ever made with the comp set). **Pricing reads the raw comp**, never the resolver: a copy or a hand-made match implants as the preset but is not trader stock, and pricing it as such would make buy-one-duplicate-many a silver printer. Also skipped: patching the duplicator itself — a foreign-target patch (deferred-application hazard above) that fixes nothing already sitting in saves.
...
| `XenogermIdentity` | Which preset a xenogerm yields: `CompXenotypeSource` when set, else gene-set inference with name tie-break |
