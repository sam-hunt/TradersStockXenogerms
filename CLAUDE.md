...
│   └── StaticConstructorOnStartupUtility_CallAll_Patch.cs  # Stamps blocksGermlineRetarget onto VRE Lycanthrope's generated morph genes
├── Stats/
...
| `GeneExtension` | `DefModExtension` for GeneDefs; `excludeFromXenogermStock` bars any xenotype containing the gene, `blocksGermlineRetarget` only shields a carrier pawn's germline from the retarget |
...
The extension's second flag, `blocksGermlineRetarget`, is the weaker opt-out: it only stops the implant patch's germline retarget on pawns carrying the gene as an endogene, leaving the xenotype sellable (`IsCandidate` never reads it; `excludeFromXenogermStock` implies it). VRE Lycanthrope's morph genes get it stamped at runtime by `StaticConstructorOnStartupUtility_CallAll_Patch` — they are generated defs (`VRE_Morphs_<XenotypeDefName>`, one per loaded xenotype, built by a Lycanthrope postfix on `GeneDefGenerator.ImpliedGeneDefs` with a hard-coded modExtensions list) that no XML patch can reach, matched by the same `VRE_Morphs_` defName prefix Lycanthrope's own code uses. The stamp rides `CallAll`, not `[StaticConstructorOnStartup]`, because it is def-instance state that must survive an in-process play-data reload (the `XenotypeLedgerStartup` rule).
