...
# Per-repo constants. This script's structure mirrors the copy in
# shunter.uniquemeleeweapons and its sibling repos (same L10nProbe-driven
# refresh flow); unlike that trio, this mod is not part of a probed family —
# it is the only sidecar-bearing mod in its own boot.
PACKAGE_ID = "shunter.tradersstockxenogerms"
...
#     Biotech is a hard dependency (About.xml's modDependencies); no other
#     DLC gates any content here;
#   * hard dependencies (Harmony);
#   * this repo's own sidecar-bearing mod. Unlike the UniqueMeleeWeapons
#     family (three independent mods riding along in one boot for
#     convenience), this mod is not part of a probed family, so the list has
#     no siblings to add;
#   * a third-party mod ONLY if this mod's content MayRequires it. Gene
#     Trader (tac.genetrader) is soft-patched (Patches_GeneTrader.xml,
#     PatchOperationFindMod) but adds no label/description — no translatable
#     keys depend on it being active, so it is left out (none needed today).
