...
# Usage:
#   ./test-windows.sh
#   ./test-windows.sh --filter FullyQualifiedName~XenogermPricing
...
MAIN_PROJECT='Source\1.6\TradersStockXenogerms.csproj'
TEST_PROJECT='Tests\1.6\TradersStockXenogerms.Tests.csproj'

echo -e "${BLUE}=== Traders Stock Xenogerms - Windows Test Runner ===${NC}"
...
#    Stage 3 robocopies the test bin to %TEMP%\tsx-tests\ — a real local
...
powershell.exe -NoProfile -Command "
    ...
    \$LocalBin = Join-Path \$env:TEMP 'tsx-tests'
    ...
    dotnet test 'TradersStockXenogerms.Tests.dll' --nologo $EXTRA_ARGS
    exit \$LASTEXITCODE
" 2>&1 | tee "$TMPF"
STATUS=${PIPESTATUS[0]}
