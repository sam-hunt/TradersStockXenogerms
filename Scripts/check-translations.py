...
#   <TSX_ResetToDefaults>...</TSX_ResetToDefaults>
...
# DLCs the sidecar must have been generated with. Biotech is a hard
...
def def_is_active(elem, active_packages):
    ...
    # absent from the sidecar and outside the l10n surface (e.g. a
    # Biotech-gated ScenPartDef under a compat load root, should this repo
    # ever grow one like sister mod BetterTradersGuild did).
...
def check_language(lang_dirs, english_keyed, sidecar, def_roots, report):
    ...
    # root is not a language of its own: a def declared under, say,
    # 1.6/Mods/SomeOptionalMod/ can only have its injections under
    # 1.6/Mods/SomeOptionalMod/Languages/<Lang>/ (MayRequire is ignored on
    # DefInjected, so the LoadFolders gate has to be the folder), yet the
    # sidecar marks them required — checking that root standalone would
    # demand the whole Keyed set and every other def's injections from it.
    # This repo has no compat roots yet; the machinery exists so adding one
    # (as sister mod BetterTradersGuild did) doesn't require touching this
    # script.
