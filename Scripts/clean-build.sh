...
echo -e "${BLUE}=== Traders Stock Xenogerms - Clean Build ===${NC}"
echo ""

# Step 1: Clean deployed mod folder
echo -e "${YELLOW}[1/2] Cleaning mod folder...${NC}"
cd "$MOD_ROOT"
dotnet build Source/1.6/TradersStockXenogerms.csproj -t:CleanModFolder --verbosity quiet
...
dotnet build Source/1.6/TradersStockXenogerms.csproj -c Release --verbosity quiet
